...
function intialize(){
    ...
    if (localStorage.getItem("seen-file-browser-demo") && window.innerWidth >= 1000){
        ...
        client_video_list.push(new DemonstrationObject("/static/video/create_folder.mp4","Create Folder","Create a folder by right clicking anyone on the screen within the files page, and then click the New Folder option. Make sure no file is selected otherwise options to the selected file will be show instead. A Folder name should be unique."))
        client_video_list.push(new DemonstrationObject("/static/video/upload_file.mp4","Upload File","Upload a file by right clicking anyone on the screen, without a file selected. The click the Upload button which will open a file browser. Upon selecting a file an upload modal will be shown which you can use to define access control measures to that file. The click the upload button."))
        client_video_list.push(new DemonstrationObject("/static/video/download_file.mp4","Download File" ,"Download a file by simply double clicking the file, which will open the download modal showing the progress of file download. As soon as the progress completes, a download will be intitiated from the main memory to your downloads folder."))
        client_video_list.push(new DemonstrationObject("/static/video/upload_modal.mp4","Open Upload Modal" ,"The upload modal is used to view the progress and details of an uploading file. Open it by pressing the key combination Ctrl + U "))
        client_video_list.push(new DemonstrationObject("/static/video/download_modal.mp4","Open Download Modal" ,"The download modal is used toshow the progress of a downloading file. Open it byt pressing the key combination Ctrl + D .Multiple file downloading is unsupported."))
        client_video_list.push(new DemonstrationObject("/static/video/access_control.mp4","Access Control Modal" ,"Open the file download modal for a file by left clicking the file and the right clicking the same file to show the file options. Select the option ACCESS CONTROL, which will open up a the file access control modal.The modal can be used to set the client account that have access to the file."))
        client_video_list.push(new DemonstrationObject("/static/video/reload_index.mp4","Reload Index" ,"Reload the index by right clicking annywhere on the screen to show the options modal, and the click the option RELOAD INDEX. Make sure no file is selected otherwise the file options menu is show instead."))
